...
- What if the private key of the device is compromised? The user can revoke the device's credentials via BLE, which wipes the private key along with the CSR and the certificate. The next CSR read generates a new key pair, and the Identity Service automatically revokes the previous certificate associated with that device. More details are covered in the [Certificate Lifecycle](./certificate-lifecycle.md) section.
